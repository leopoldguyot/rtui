...
vignette: >
  %\VignetteIndexEntry{Reactivity reference}
...
- Dependencies are captured dynamically as reactive code reads inputs/reactives.
...
Reactive value that updates only when `event` triggers.
Useful for “Apply” button semantics.
...
Short-circuits when values are falsy.
In renderers, output resolution is skipped for that cycle.
In reactives, previous stable value is retained when available.
